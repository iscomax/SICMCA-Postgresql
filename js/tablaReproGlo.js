$(document).ready(function() {
     var tablaReporteGlo=$('#tablaGlo').DataTable({
  ...
  Highcharts.setOptions({
   ...
   var chart = Highcharts.chart(graficaRG[0], {
     chart: {
         type: 'column'
       },
       title: {
    ...
    tooltip: {
        headerFormat: 'Actividad',
        pointFormat: '<span style="color:{point.color}">\u25CF</span> <b> {point.name}</b><br/>' +
          'Pendientes: <b>{point.y}</b><br/>'
      },
     ...
     area: {
       ...
       series: [{
           ...
           dataSorting: {
             enabled: true},  
           colorByPoint: true,
           data: chartData(tablaReporteGlo)//llamada a los datos almacenados
       }]
 
   });
 
 //función chartData
 function chartData(tablaReporteGlo) {
     var filasAfectadas = {};//inicializacion de la variable para el conteo de las filas afectadas
     tablaReporteGlo.column(12, {
